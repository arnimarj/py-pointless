...
class TestThreadSafe(unittest.TestCase):
	...
	def testThreadedProjSort(self):
		...
		for i in six.moves.range(10):
			v_ = pointless.PointlessPrimVector('u32', sequence = (random.randint(0, 100) for i in six.moves.range(N_DOMAIN)), allow_print = False)
			v.append(v_)

		t = pointless.PointlessPrimVector('u32', sequence = (random.randint(0, N_DOMAIN - 1) for i in six.moves.range(N_ITEMS)), allow_print = False)
		p = [ ]
